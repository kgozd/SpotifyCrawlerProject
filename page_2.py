...
from customtkinter import(
    ...
    CTkScrollableFrame,
    CTkTextbox, CTkLabel, CTkFont, CTkImage,CTkFrame,CTk,CTkOptionMenu
)  
from io import BytesIO
from PIL import ImageTk, Image, ImageDraw
from urllib.request import urlopen
from os.path import dirname, join, abspath
from sqlite3 import OperationalError, Cursor,connect
...
class Wykres:
    ...
    def rysuj(self, plot_title, categories_title, values_title, bars_titles, bars_values  ):
       # bars_values = []
        ...
        self.ax.autoscale()
        # self.ax.bar_label(
        #     slupki, fmt=lambda x: '{:.2f}'.format(x ), color='white', zorder=2,label_type='edge'
        # )
        #'{:,.0f}'
        ...
        # max_value = max(bars_values)
        # y_upper_limit = int(round(max_value + max_value/5 ))  # Dodatkowy odstęp 1
        # print(y_upper_limit)
        # self.ax.set_ylim(0, y_upper_limit)
        self.ax.tick_params(axis='x', colors='white')  # Zmiana koloru zaznaczeń na osi X na niebieski
        self.ax.tick_params(axis='y', colors='white')  # Zmiana koloru zaznaczeń na osi Y na zielony
...
class Page2(Page):
    ...
    def create_widgets(self):
        ...
        scrollbar_frame.grid(row=1, column=2, padx=(0, 0), pady=(0, 0), sticky="ns")
        ...
        self.optionmenu = CTkOptionMenu(self, dynamic_resizing=True,
                                        ...
                                        command=self.handle_optionmenu_selection, font=("Arial", 17,'bold'), height=40,fg_color="#4ddf5d",
                                           text_color="#000000" ,  text_color_disabled= "#111111",dropdown_font=("Arial", 14,'bold'),dropdown_text_color="white",button_color="#4ddf5d",button_hover_color="#3bac47")
    ...
    def track_selection(self, event):
        ...
        print( self.selected_track_name)
    ...
    def set_plot_values(self):
        try:
            ...
            percent= abs((float(query_1) - float(query_2))*100)
            percent = "{:.2f}".format(percent)
            if float(query_1) > float(query_2):
                
                self.title= self.my_labels.create_label(f" {self.selected_key.get()} w utworze  jest o {percent}% mniejsza niż średnia dla albumu",
                                                        row=5, column=2, padx=(20, 20), pady=(5, 5), columnspan = 5) 
            else:
                self.title= self.my_labels.create_label(f"{self.selected_key.get()} w utworze  jest o {percent}% większa niż średnia dla albumu",
                                                                                     row=5, column=2, padx=(20, 20), pady=(5, 5), columnspan = 5)                                                                                                                                             
        ...
        try:
            ...
            self.plot1.rysuj(f"{key} dla utworu {utwor[0:15]}{elo}", "", "",[ "Średnia albumu","Wybrany utwór"], [round(float(query_2),2),round(float(query_1),2)])
